...
3. # Learning the parameters of utility and loss functions of an agent.
...
Consider a boat moving in $\mathbf{R}^2$, from a starting point $x(0) = y(0) = 0$, to $x(1) = 1, y(1) = 0$ with a model: $$x' = V cos(u) , y' = V sin(u) $$ and let us assume the driver wants to minimise the time spent around the apex of the corner, which can be expressed by the loss function as $$J = \int_0^1 y dt $$. One can show that a trajectory satifying the Pontryagin Minimum Principle will satisfy: $tan(u(t)) = At + b $ for some real values A,B. 

The research question I would like to work on next is: 
Given a sample trajectory: X[t] how quickly can we establish that the loss function of this agent is different from our prior assumption, how quickly can we model it, and act upon it in presence of uncertainty i.e. optimise our own strategy for the corner, in case it also depends on the distance from the observed car. 
